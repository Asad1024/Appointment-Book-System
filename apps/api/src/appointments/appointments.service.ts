...
export class AppointmentsService {
  ...
  async book(
    ...
    try {
      ...
      void this.calendarSync.onAppointmentBooked(result.id);
  ...
  async listWaitlist(organizationId: string, locationId?: string) {
  ...
  /** Calendar date in the location timezone (matches booking UI `preferredDate`). */
  private preferredDateForSlot(startUtc: Date, timezone: string): string {
    return DateTime.fromJSDate(startUtc, { zone: 'utc' }).setZone(timezone).toFormat('yyyy-MM-dd');
  }

  private async notifyWaitlistForReleasedSlot(params: {
    serviceId: string;
    providerId: string;
    locationId: string;
    startUtc: Date;
    ...
    const preferredDate = this.preferredDateForSlot(params.startUtc, params.timezone);
    ...
    for (const w of waiters) {
      try {
        await this.email.send(
          w.customerEmail,
          'A slot opened up — book now',
          `<p>Hi ${w.customerName},</p>` +
            `<p>A time slot for <strong>${serviceName}</strong> on <strong>${preferredDate}</strong> is now available.</p>` +
            `<p><a href="${bookUrl.toString()}">Book now</a></p>`,
        );
        ...
        this.logger.error(`Waitlist notify failed for ${w.id} (${w.customerEmail})`, e);
      }
    }

    if (notified > 0 && loc?.organizationId) {
      this.realtime.emit(loc.organizationId, { type: 'waitlist.updated' });
    }

    this.logger.log(
      `Waitlist: notified ${notified}/${waiters.length} for ${preferredDate} service=${params.serviceId}`,
    );
    return notified;
  }
  ...
  private async notifyWaitlist(appt: {
    ...
    try {
      await this.notifyWaitlistForReleasedSlot({

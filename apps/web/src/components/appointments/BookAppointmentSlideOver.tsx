...
export function BookAppointmentSlideOver({
  ...
  async function copyLink() {
    if (!bookingUrl) {
      toast.error('Select service and provider first.');
      return;
    }
    try {
  ...
  async function createAppointment() {
    ...
    if (cleanPhone.length < 8 || cleanPhone.length > 20 || !PHONE_RE.test(cleanPhone)) {
      setError('Enter a valid customer phone number (e.g. +971501234567).');
      return;
    }

...
const pricingPlans: {
...
}[] = [
  {
    ...
    icon: Wallet,
    limits: [
      { label: 'Bookings / month', value: '25' },
      { label: 'Staff accounts', value: '2' },
      { label: 'Locations', value: '1' },
      { label: 'Services', value: '5' },
    ],
    features: ['Booking page + reminders', 'Admin + staff dashboard'],
    ctaLabel: 'Start free',
    ctaHref: '/signup',
  },
  {
  ...
  },
  {
    name: 'Scale',
    price: 'Custom',
    subtitle: 'For multi-location operations',
    icon: Building2,
    limits: [
      { label: 'Bookings / month', value: '10,000+' },
      { label: 'Staff accounts', value: 'Unlimited' },
      { label: 'Locations', value: 'Unlimited' },
      { label: 'Services', value: 'Unlimited' },
    ],
    features: ['Launch support', 'Custom onboarding'],
...
function PricingPlanCard({
  ...
  return (
    <motion.article
      ...
      transition={{ duration: 0.35, delay: index * 0.05 }}
      className={cn(
        'rounded-2xl border bg-white p-5 shadow-card dark:bg-slate-900',
        featured
          ? 'border-brand-300 ring-1 ring-brand-200 dark:border-brand-700 dark:ring-brand-900/60'
          : 'border-slate-200 dark:border-slate-800',
      )}
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-text-muted">{name}</p>
          <h3 className="mt-2 font-display text-3xl font-bold text-text-primary">{price}</h3>
        ...
        <div
          className={cn(
            ...
            featured
              ? 'border-brand-200 bg-brand-50 text-brand-700 dark:border-brand-700 dark:bg-brand-900/25 dark:text-brand-300'
      ...
      <div className="mt-5 grid gap-2">
        {limits.map((item) => (
          <div
            key={`${name}-${item.label}`}
            className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm dark:border-slate-800 dark:bg-slate-950/60"
          >
      ...
      <Link href={ctaHref} className="mt-5 block">
        <Button
          ...
          className={cn('w-full', !featured && 'border-brand-200 text-brand-700 hover:bg-brand-50')}
        >
          {ctaLabel}
        </Button>
      </Link>
  ...
  return (
    <PageTransition>
      <div className="overflow-hidden">
        ...
        <section className="border-b border-slate-200/80 bg-white py-14 sm:py-16 dark:border-slate-800 dark:bg-slate-950">
          <div className="mx-auto max-w-6xl px-5 sm:px-6">
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <h2 className="mt-3 font-display text-3xl font-bold tracking-tight text-text-primary">
                Start free, upgrade as your demand grows
              </h2>
              <p className="mt-3 text-base text-text-secondary">
                Clear limits for bookings, team size, locations, and services.
              </p>
            </motion.div>

            <div className="mt-10 grid gap-4 lg:grid-cols-3">
              {pricingPlans.map((plan, index) => (
                <PricingPlanCard key={plan.name} index={index} {...plan} />
              ))}
            </div>
          </div>
        </section>

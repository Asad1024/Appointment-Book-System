...
import { calendarEventFromParts, downloadIcsFile } from '@/lib/calendar-export';
...
function ManageAppointmentBody({
  appt,
  token,
  reviewMeta,
  error,
  ...
  selectedDate: string;
  displaySlots: SlotOption[];
  newStartUtc: string;
  customerTimezone: string;
  setCustomerTimezone: (tz: string) => void;
  minBookDate: string;
  maxBookDate: string;
  openRescheduleMode: () => void;
  closeRescheduleMode: () => void;
  setSelectedDate: (d: string) => void;
  setNewStartUtc: (utc: string) => void;
  reschedule: () => void | Promise<void>;
  canChangeTime: boolean;
  downloadIcs: () => void;
  setCancelOpen: (open: boolean) => void;
  setReviewMeta: (meta: ReviewMeta | null) => void;
}) {
  return (
    <div className="grid gap-8 lg:grid-cols-3">
      ...
      <ManageAppointmentSidebar
        ...
        onDownloadIcs={downloadIcs}
        canModify={appt.status !== 'cancelled' && appt.status !== 'completed'}
        canChangeTime={canChangeTime}
        reschedulesLeft={Math.max(0, 3 - appt.rescheduleCount)}
      />
    </div>
  );
}
...
function ManagePageContent() {
  ...
  const isCustomerPortal = !partner && Boolean(user && !isStaff);

  const appointmentBodyProps = appt
    ? {
        appt,
        token: token ?? '',
        reviewMeta,
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="mt-1 text-sm text-text-secondary sm:text-base">
              View your session, reschedule, cancel, or add it to your calendar.
            </p>
          </div>
          ...
          {error && !appt && !pageLoading && (
            <Card className="max-w-lg">
              <CardBody>
                ...
                <Link href="/account" className="mt-4 inline-block text-sm font-medium text-brand-600 hover:underline">
                  Back to my appointments
                </Link>
              </CardBody>
            </Card>
          ...
          {appointmentBodyProps ? <ManageAppointmentBody {...appointmentBodyProps} /> : null}
        </div>

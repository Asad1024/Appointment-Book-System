...
export default function AdminProvidersPage() {
  ...
  useEffect(() => {
    const onLimitsUpdated = () => void load();
  ...
  const filteredActiveProviders = useMemo(() => {
    ...
    return activeProviders.filter((provider) => {
      const matchesSearch =
        q.length === 0
          ? true
          : [provider.name, provider.email ?? '', provider.phone ?? '', provider.location?.name ?? '']
              .join(' ')
              .toLowerCase()
              .includes(q);
      const matchesStatus =
  ...
  function openEdit(p: Provider) {
    setOpenMenuId(null);
    setEditing(p);
    setForm({ name: p.name, email: p.email ?? '', phone: p.phone ?? '', isActive: p.isActive });
  ...
  async function saveProvider(e: React.FormEvent) {
    ...
    if (trimmedPhone && !isValidPhoneValue(trimmedPhone)) {
        ...
        if (created.invitePending) {
          toast.success(
            created.inviteEmailSent
              ? 'Staff created as inactive. Invite email sent. After acceptance they sign in at /staff/login.'
              : 'Staff created as inactive. Invite created, but email could not be sent.',
          );
        } else {
  ...
  async function runProviderAction() {
    ...
    try {
      ...
      toast.success(providerAction.type === 'archive' ? 'Staff archived' : 'Staff deleted');
      await load();
    } catch (e) {
  ...
  async function toggleActive(p: Provider) {
    try {
      ...
      toast.success(p.isActive ? 'Staff paused' : 'Staff activated');
      await load();
    } catch (e) {
  ...
  async function resendInvite() {
    if (!editing) return;
    if (form.isActive) {
      toast.error('Only inactive staff can receive an invite email');
      return;
    }
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              {p.phone ? (
                ...
                <p className="mt-1 text-xs text-text-muted">No phone provided</p>
              )}
            ...
            >
              <CatalogStatusBadge
                ...
                invitePending={p.invitePending}
                planSuspended={p.planSuspended}
              />
  ...
  return (
    <PageTransition>
      <div className="-mx-4 -mt-4 sm:-mx-8 sm:-mt-8">
        ...
        <div className="px-4 pb-6 sm:px-5 lg:px-6">
          <div className="mb-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            <Card className="border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <CardBody className="p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-text-secondary">Total staff</p>
                    <p className="mt-1 text-xs text-text-muted">All team staff</p>
                  </div>
          ...
          <ResourceListToolbar
            ...
            summary={`${totalProvidersCount} staff${showArchived && archivedCount > 0 ? ` - ${archivedCount} archived` : ''}`}
            filters={[
              {
                id: 'providers-status',
                label: 'Status',
          ...
          {loading ? (
            ...
            <div className="rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950">
              <EmptyState
                icon={Users}
                title="No staff yet"
                description="Add staff to assign schedules and bookings."
                action={
                  <Button onClick={openNew}>
                    <Plus className="mr-2 h-4 w-4" />
                    New staff
      ...
      <SlideOver
        ...
        description={
          editing
            ? 'Update staff details, status, and service assignments'
        ...
        <form className="space-y-4" onSubmit={saveProvider}>
          ...
          <div className="relative z-10">
            ...
            <p className="mt-1 text-xs text-text-secondary">
              Booking confirmations and reminders are sent to this number when configured.
            </p>
          </div>
          ...
          {editing ? (
            <div className="flex items-center justify-between rounded-xl border border-slate-100 bg-surface-subtle px-4 py-3 dark:border-slate-800">
              <div>
                ...
                <p className="text-xs text-text-secondary">
                  Paused staff stay on your team but are hidden from booking
                </p>
              </div>
              <Switch
            ...
            <div className="rounded-xl border border-slate-100 bg-surface-subtle px-4 py-3 dark:border-slate-800">
              <p className="text-sm font-semibold text-text-primary">Staff activation</p>
              <p className="text-xs text-text-secondary">
                Staff with email stay inactive until they accept their invite.
                Staff without email are created active. After accepting invite, staff sign
          ...
          {editing && !form.isActive ? (
            <div className="rounded-xl border border-slate-100 bg-surface-subtle px-4 py-3 dark:border-slate-800">
              ...
              <p className="text-xs text-text-secondary">
                If the email is wrong, update it above and resend the staff invite.
              </p>
              <Button
                type="button"
                variant="outline"
          ...
          {editing ? (
            ...
            <div className="rounded-xl border border-slate-100 bg-surface-subtle p-4 dark:border-slate-800">
              <h3 className="text-sm font-semibold text-text-primary">Assign services</h3>
              <p className="mt-0.5 text-xs text-text-secondary">
                Save the staff member first, then assign services.
              </p>
            </div>
          )}
      ...
      <ConfirmDialog
        open={!!providerAction}
        onOpenChange={(o) => !o && !providerActionLoading && setProviderAction(null)}
        title={providerAction?.type === 'archive' ? 'Archive staff?' : 'Delete staff?'}
        description={
          providerAction?.type === 'archive'
            ? 'Archived staff are hidden from booking and scheduling. You can restore them later.'
            : 'This permanently deletes the staff member and linked login (only if no appointment history exists).'
        }
        confirmLabel={providerAction?.type === 'archive' ? 'Archive' : 'Delete'}
        loadingLabel={providerAction?.type === 'archive' ? 'Archiving...' : 'Deleting...'}
        variant="destructive"
        loading={providerActionLoading}

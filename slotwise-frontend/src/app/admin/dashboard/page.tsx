...
export default function AdminDashboardPage() {
  ...
  const [calendarFocusUtc, setCalendarFocusUtc] = useState<string | null>(null);
  const [calendarRangeReady, setCalendarRangeReady] = useState(false);
  const loadSeqRef = useRef(0);
  const weekStartRef = useRef(weekStart);
  const weekEndRef = useRef('');
  const tz = useMemo(
  ...
  const jumpToWeekOfAppointment = useCallback(
    (startUtc: string) => {
      ...
      if (dashboardView === 'calendar') {
        setCalendarFocusUtc(startUtc);
      }
      // List view uses weekStart/weekEnd. Calendar fetch range is owned by AppointmentCalendar.onRangeChange.
    },
    [dashboardView, tz],
  );

  const loadAppointments = useCallback(async () => {
    if (dashboardView === 'calendar' && !calendarRangeReady) return;

    const seq = ++loadSeqRef.current;
    setLoading(true);
    try {
      const from = dashboardView === 'calendar' ? rangeFrom : weekStartRef.current;
      const to = dashboardView === 'calendar' ? rangeTo : weekEndRef.current;
  ...
  }, [calendarRangeReady, dashboardView, rangeFrom, rangeTo, statusFilter, locationId]);
  ...
  function setView(v: DashboardView) {
    ...
    if (v === 'calendar') {
      setCalendarRangeReady(false);
    } else {
      setCalendarRangeReady(true);
    }
  }

  function setStatusFilterWithLoader(value: string) {
    setLoading(true);
    setStatusFilter(value);
  }

  function setWeekStartWithLoader(value: string) {
    setLoading(true);
    setWeekStart(value);
  }
  ...
  async function notifyWaitlistEntry(id: string) {
    ...
    try {
      ...
      toast.success('Waitlist customer notified by email');
      void loadWaitlist();
    } catch (e) {
  ...
  function shiftWeek(delta: number) {
    setWeekStartWithLoader(format(addDays(parseISO(weekStart), delta * 7), 'yyyy-MM-dd'));
  }
      ...
      <Tabs defaultValue="appointments" onValueChange={(v) => v === 'waitlist' && void loadWaitlist()}>
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
            ...
            {dashboardView === 'calendar' && (
              <Select value={statusFilter} onValueChange={setStatusFilterWithLoader}>
        ...
        <TabsContent value="appointments" className="mt-0 focus-visible:outline-none">
          {dashboardView === 'calendar' ? (
            <AppointmentCalendar
              ...
              focusStartUtc={calendarFocusUtc}
              onRangeChange={(from, to) => {
                if (!calendarRangeReady || from !== rangeFrom || to !== rangeTo) {
                  setLoading(true);
                }
                setRangeFrom(from);
                setRangeTo(to);
                ...
                <div className="flex flex-col gap-4 rounded-2xl border border-slate-200/80 bg-gradient-to-br from-white via-white to-slate-50 p-4 dark:border-slate-800 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800/70 lg:flex-row lg:items-end lg:justify-between">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Button
                      ...
                      variant="secondary"
                      size="sm"
                      onClick={() => setWeekStartWithLoader(format(weekStartMonday(new Date()), 'yyyy-MM-dd'))}
                    >
                      Today
                    </Button>
                  </div>
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
                    <div>
                      ...
                      <Input
                        ...
                        type="date"
                        value={weekStart}
                        onChange={(e) => setWeekStartWithLoader(e.target.value)}
                    ...
                    <div className="space-y-3 md:hidden">
                      {filtered.map((a) => (
                        ...
                        >
                          ...
                          <p className="mt-1 text-sm text-text-muted">Staff: {a.provider.name}</p>
                    ...
                    <table className="w-full text-left text-sm">
                      ...
                      <tbody className="divide-y divide-slate-100 bg-white dark:divide-slate-800 dark:bg-slate-950">
                        {waitlist.map((w) => {
                          ...
                          const busy = waitlistActionId === w.id;
                          return (
                                ...
                                <div className="flex justify-end gap-2">
                                  {canNotify && (
                                    <Button
                                      ...
                                      variant="outline"
                                      disabled={busy}
                                      onClick={() => void notifyWaitlistEntry(w.id)}
                                    >
                                  ...
                                  {(w.status === 'active' || w.status === 'notified') && (
                                    <Button
                                      ...
                                      variant="ghost"
                                      disabled={busy}
                                      onClick={() => void removeWaitlistEntry(w.id)}
                                    >
                    ...
                    {waitlist.map((w) => (
                      <div key={w.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                        ...
                        <p className="mt-2 text-sm text-text-muted">
                          {w.preferredDate} · {w.preferredTimeLabel} · {w.provider?.name ?? 'Any'}
                        </p>

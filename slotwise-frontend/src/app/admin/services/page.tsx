...
const SERVICE_PAYMENTS_ENABLED = false;
...
export default function AdminServicesPage() {
  ...
  async function generateDescription() {
    ...
    try {
      const response = await apiAuth<{ description: string }>('/ai/service-description', {
        method: 'POST',
        body: JSON.stringify({
          name,
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-xs text-text-secondary">
                ...
                {SERVICE_PAYMENTS_ENABLED
                  ? ` - ${formatServicePrice(s.priceCents, bookingCurrency)}`
                  : ''}
              </p>
          ...
          <table className="w-full table-fixed text-sm">
            <thead className="bg-slate-50/80 dark:bg-slate-900/70">
              <tr className="text-left text-xs font-semibold uppercase tracking-wide text-text-secondary">
                <th className={SERVICE_PAYMENTS_ENABLED ? 'w-[32%] px-4 py-3' : 'w-[40%] px-4 py-3'}>Name</th>
                <th className="w-[14%] px-4 py-3">Duration</th>
                {SERVICE_PAYMENTS_ENABLED && <th className="w-[14%] px-4 py-3">Price</th>}
  ...
  return (
    <PageTransition>
          ...
          <div className="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-5 lg:px-6">
            <div>
              ...
              <p className="mt-1 text-sm text-text-secondary">
                Manage bookable services and staff assignments
              </p>
            </div>
              ...
              <CardBody className="p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-text-muted">All configured services</p>
                  </div>
              ...
              <CardBody className="p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-text-muted">{totalServicesCount - bookableCount} paused</p>
                  </div>
                ...
                <CardBody className="p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-text-muted">Require online payment</p>
                    </div>
      ...
      <SlideOver
        ...
        description={editing ? 'Update service details and staff assignments' : 'Add a bookable service'}
      >
            ...
            <form className="space-y-4" onSubmit={save}>
              ...
              <div>
                ...
                <p className="mt-1 text-right text-xs text-text-muted">
                  {form.description.length}/{SERVICE_DESCRIPTION_MAX_LENGTH}
                </p>
              </div>
              ...
              <div className="flex items-center justify-between rounded-xl border border-slate-100 bg-surface-subtle px-4 py-3 dark:border-slate-800">
                <div>
                  ...
                  <p className="text-xs text-text-secondary">
                    Auto-assign this service to each newly created staff member.
                  </p>
                </div>
                <Switch
              ...
              {editing ? (
                (() => {
                  ...
                  return (
                    <div className="rounded-xl border border-slate-100 bg-surface-subtle p-4 dark:border-slate-800">
                      <h3 className="text-sm font-semibold text-text-primary">Staff assignments</h3>
                      <p className="mt-0.5 text-xs text-text-secondary">
                ...
                <div className="rounded-xl border border-slate-100 bg-surface-subtle p-4 dark:border-slate-800">
                  <h3 className="text-sm font-semibold text-text-primary">Staff assignments</h3>
                  <p className="mt-0.5 text-xs text-text-secondary">
                    Save the service first, then assign staff.
                  </p>
                </div>
              )}
      ...
      <ConfirmDialog
        open={!!serviceAction}
        onOpenChange={(o) => !o && !serviceActionLoading && setServiceAction(null)}
        ...
        loadingLabel={serviceAction?.type === 'archive' ? 'Archiving...' : 'Deleting...'}
        variant="destructive"

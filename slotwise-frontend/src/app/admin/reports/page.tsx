...
const REPORT_REVENUE_ENABLED = false;

const chartLoading = () => <Skeleton className="h-72 w-full rounded-xl" />;

const AdminReportsLineChart = dynamic(
  () =>
    import('@/components/admin/reports/AdminReportsCharts').then((mod) => ({
      default: mod.AdminReportsLineChart,
    })),
  { ssr: false, loading: chartLoading },
);

const AdminReportsStatusPieChart = dynamic(
  () =>
    import('@/components/admin/reports/AdminReportsCharts').then((mod) => ({
      default: mod.AdminReportsStatusPieChart,
    })),
  { ssr: false, loading: chartLoading },
);

const AdminReportsServiceBarChart = dynamic(
  () =>
    import('@/components/admin/reports/AdminReportsCharts').then((mod) => ({
      default: mod.AdminReportsServiceBarChart,
    })),
...
function KpiSection({
  ...
  const cards = [
    ...
    },
    {
      ...
      sub: `${s.cancellationRate.toFixed(1)}% cancellation rate`,
    },
    {
      label: 'No-shows',
      value: s.noShows,
      ...
      trend: null,
      sub: `${s.noShowRate.toFixed(1)}% no-show rate`,
    },
    {
  ...
  return (
    <div
      className={cn(
        ...
        REPORT_REVENUE_ENABLED ? '2xl:grid-cols-7' : '2xl:grid-cols-6',
      )}
    >
      {cards.map((kpi) => (
        <Card key={kpi.label}>
...
export default function AdminReportsPage() {
  ...
  const [byService, setByService] = useState<SectionState<ByServiceRow[]>>(
    REPORT_REVENUE_ENABLED ? emptySection : () => ({ data: [], loading: false, error: null }),
  );
  ...
  const isFetching =
    summary.loading ||
    byDay.loading ||
    (REPORT_REVENUE_ENABLED && byService.loading) ||
    byProvider.loading ||
    peakHours.loading;
  ...
  const fetchByProvider = useCallback(async () => {
    ...
    try {
      ...
      setByProvider({
        data: null,
        loading: false,
        error: e instanceof Error ? e.message : 'Failed to load staff performance',
      });
    }
  }, [queryString]);
  ...
  const refreshAll = useCallback(() => {
    void fetchSummary();
    void fetchByDay();
    if (REPORT_REVENUE_ENABLED) void fetchByService();
  ...
  return (
    <PageTransition>
      ...
      <div className="mb-8 grid gap-6 lg:grid-cols-2">
        <Card>
          <CardBody>
            <SectionBlock
              ...
              onRetry={fetchByDay}
            >
              <AdminReportsLineChart data={lineChartData} />
            </SectionBlock>
          </CardBody>
        </Card>
        ...
        <Card>
          <CardBody>
            ...
            >
              <AdminReportsStatusPieChart data={statusPieData} />
            </SectionBlock>
          </CardBody>
        </Card>
      </div>
      ...
      <div
        className={cn(
          'mb-8 grid gap-6',
          REPORT_REVENUE_ENABLED ? 'lg:grid-cols-2' : 'lg:grid-cols-1',
        )}
      >
        {REPORT_REVENUE_ENABLED ? (
          <Card>
            <CardBody>
              ...
              >
                <AdminReportsServiceBarChart data={serviceChartData} currency={CURRENCY} />
              </SectionBlock>
            </CardBody>
          </Card>
        ) : null}
        ...
        <Card>
          <CardBody>
            <SectionBlock
              ...
              description="Sort columns to compare staff"
            ...
            >
              {sortedProviders.length === 0 ? (
                <p className="text-sm text-text-secondary">No staff data in this period.</p>
              ) : (

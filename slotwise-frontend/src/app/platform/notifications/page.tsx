...
const READ_STORAGE_KEY = 'slotwise_platform_notifications_read_ids';
const DELETED_STORAGE_KEY = 'slotwise_platform_notifications_deleted_ids';
...
export default function PlatformNotificationsPage() {
  ...
  const scopeOrgId = searchParams.get('orgId') ?? '';
  const scopeOrgSearch = searchParams.get('search') ?? '';
  const scopeOrgStatus = searchParams.get('status') ?? '';
  ...
  const buildQuery = useCallback(() => {
    ...
    if (scopeOrgId) q.set('orgId', scopeOrgId);
    if (scopeOrgSearch) q.set('search', scopeOrgSearch);
    if (scopeOrgStatus) q.set('status', scopeOrgStatus);
    if (status !== 'all') q.set('deliveryStatus', status);
    if (channel !== 'all') q.set('channel', channel);
    if (searchQuery) q.set('q', searchQuery);
    return q;
  }, [scopeOrgId, scopeOrgSearch, scopeOrgStatus, status, channel, searchQuery]);
  ...
  const load = useCallback(
    async (withLoader = true) => {
      ...
      try {
        const data = await apiAuth<NotificationResponse>(
          `/platform/notifications?${buildQuery().toString()}`,
        );
        setItems(data.items);
      } catch (e) {
          ...
          <div className="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-5 lg:px-6">
            <div>
              ...
              <p className="mt-1 text-sm text-text-secondary">
                Delivery activity across organizations
              </p>
            ...
            <Card className="border-slate-200 shadow-sm dark:border-slate-800">
              <CardBody>
                <EmptyState
                  ...
                  description="Cross-tenant notification activity will appear here."
                />
              </CardBody>
            </Card>
          ) : (
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <div className="min-w-0">
                              ...
                              <p className="mt-1 truncate text-xs text-text-muted">
                                {item.appointment.organizationName} - {item.appointment.locationName}
                              </p>
                            </div>
                          </td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex flex-nowrap items-center justify-end gap-1.5">
                              <Link
                                href={`/platform/organizations/${item.appointment.organizationId}`}

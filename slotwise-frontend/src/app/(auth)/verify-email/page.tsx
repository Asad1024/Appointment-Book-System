...
import { resolveCustomerLandingPath, resolveCustomerPath, withTenantPath } from '@/lib/resolve-org-slug';
...
type VerificationResponse = {
  message?: string;
  user?: { role: string; organizationSlug?: string | null };
  alreadyVerified?: boolean;
};

const verificationRequests = new Map<string, Promise<VerificationResponse>>();

function verifyEmailOnce(params: URLSearchParams) {
  const key = params.toString();
  const existing = verificationRequests.get(key);
  if (existing) return existing;

  const request = api<VerificationResponse>(`/auth/verify-email?${key}`)
    .then((response) => {
      window.setTimeout(() => verificationRequests.delete(key), 10000);
      return response;
    })
    .catch((error) => {
      ...
      .then((res) => {
        ...
        const destination =
          res.user?.role === 'customer'
            ? resolveCustomerPath(search, '/account', res.user.organizationSlug ?? orgHint)
            : res.user?.role

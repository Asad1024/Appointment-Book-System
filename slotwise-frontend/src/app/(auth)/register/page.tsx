...
function RegisterFormContent() {
  ...
  async function onSubmit(values: RegisterForm) {
    try {
      ...
      if (result.requiresEmailVerification) {
        ...
        router.push(withTenantPath(`/verify-email?${verifyParams.toString()}`, orgSlug));
      } else {
        router.push(withTenantPath('/customer/login', orgSlug));
      }
    } catch (err) {
  ...
  const customerLogoHref = resolveCustomerLandingPath(search, orgSlug);

  return (
    <AuthShell
      title="Create your account"
      subtitle="Track appointments with this provider — use the same email you book with"
      logoHref={customerLogoHref}
    >
      ...
      <p className="mt-4 text-center text-sm text-text-secondary">
        ...
        <Link
          href={withTenantPath('/customer/login', orgSlug)}

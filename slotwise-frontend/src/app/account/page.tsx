...
export default function AccountPage() {
  ...
  const [waitlistLeavingId, setWaitlistLeavingId] = useState<string | null>(null);
  ...
  const loadWaitlist = useCallback(async () => {
    try {
      const rows = await apiAuth<CustomerWaitlistEntry[]>('/appointments/waitlist/mine');
  ...
  }, [router, loadAppointments, loadWaitlist, searchParams]);

  async function leaveWaitlist(id: string) {
    setWaitlistLeavingId(id);
    try {
      await apiAuth(`/appointments/waitlist/mine/${id}`, { method: 'DELETE' });
      toast.success('Removed from waitlist');
      void loadWaitlist();
    } catch (e) {
      toast.error(e instanceof Error ? e.message : 'Could not leave waitlist');
    } finally {
      setWaitlistLeavingId(null);
    }
  }
  ...
  const orgSlug = user?.organizationSlug ?? user?.organizations?.[0]?.slug ?? '';
  ...
  const assistantAccountContext = useMemo(
    () => ({
      stats,
      ...
      waitlist: waitlistEntries.slice(0, 5),
      filters: {
        search,
        statusFilter,
        timeFilter,
    ...
    <PageTransition>
      <div className="mx-auto w-full max-w-[1360px] space-y-5">
        ...
        {waitlistEntries.length > 0 && (
          <Card className="border-brand-200 bg-brand-50/40 shadow-sm dark:border-brand-900 dark:bg-brand-950/30">
            <CardBody className="space-y-3 p-4 sm:p-5">
              <div>
                <h2 className="font-display text-lg font-semibold text-text-primary">My waitlist</h2>
                <p className="text-sm text-text-secondary">
                  We will email you when a matching slot opens. Slots are not held for you.
                </p>
              </div>
              <ul className="space-y-2">
                {waitlistEntries.map((w) => (
                  <li
                    key={w.id}
                    className="flex flex-col gap-2 rounded-xl border border-brand-100 bg-white p-3 sm:flex-row sm:items-center sm:justify-between dark:border-brand-900 dark:bg-slate-950"
                  >
                    <div>
                      <p className="font-medium text-text-primary">{w.service.name}</p>
                      <p className="text-sm text-text-secondary">
                        {w.preferredDate} · {w.preferredTimeLabel} · {w.provider?.name ?? 'Any staff'}
                      </p>
                      <p className="mt-0.5 text-xs capitalize text-text-muted">{w.status}</p>
                    </div>
                    <Button
                      type="button"
                      size="sm"
                      variant="outline"
                      disabled={waitlistLeavingId === w.id}
                      onClick={() => void leaveWaitlist(w.id)}
                    >
                      Leave waitlist
                    </Button>
                  </li>
          ...
          <CardBody className="space-y-4 p-4 sm:p-5">
            <div className="grid gap-3 lg:grid-cols-[1.2fr_repeat(4,minmax(0,180px))]">
              <div className="relative">
                ...
                <Input
                  placeholder="Search service, staff, location, status..."
                  className="pl-9"
                  value={search}
      ...
      {orgSlug && (
        <CustomerAssistantChat
          context={{
            ...
            page: 'account',
            step: activeView,
            accountContext: assistantAccountContext,
          }}
        />
      )}

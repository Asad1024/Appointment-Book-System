...
export function GenerateBookingLinkSlideOver({
  ...
  initialProviderId,
  title = 'Booking link',
  description = 'Share this link so customers book a specific service with specific staff.',
}: {
  open: boolean;
  ...
  async function copyLink() {
    if (!bookingUrl) {
      toast.error('Select a service and staff first');
      return;
    }
    try {
      ...
      {!loading && options && options.pairs.length > 0 && (
        <div className="space-y-5">
          ...
          <div>
            ...
            >
              <SelectTrigger className="mt-1.5">
                <SelectValue placeholder={serviceId ? 'Choose staff' : 'Select a service first'} />
              </SelectTrigger>
              <SelectContent>

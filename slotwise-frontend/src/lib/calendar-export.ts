...
/** Open an external URL in a new tab (anchor click avoids popup blockers on `window.open`). */
...
export function openGoogleCalendar(event: CalendarEventInput): void {
  openExternalUrl(buildGoogleCalendarUrl(event));
}
...
export function calendarEventFromParts(parts: {
  ...
  return {
    title: parts.serviceName,
    startUtc: parts.startUtc,
    endUtc: parts.endUtc,
    timezone: parts.timezone,
    description: `With ${parts.providerName}`,
    location: [parts.locationName, parts.locationAddress].filter(Boolean).join(' — '),
  };
}

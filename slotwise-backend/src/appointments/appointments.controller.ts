...
import { WaitlistDto, WaitlistLeaveDto } from './dto/waitlist.dto';
...
export class AppointmentsController {
  ...
  leaveWaitlist(@Param('id') id: string, @Body() dto: WaitlistLeaveDto) {
    return this.appointments.leaveWaitlist(id, dto.customerEmail);
  }
  ...
  @Get('waitlist')
  listWaitlist(
    @Req() req: { user: { orgId: string; role: string; providerId?: string | null } },
    @Query('locationId') locationId?: string,
  ) {
    return this.appointments.listWaitlist(
      req.user.orgId,
      locationId,
      providerScope(req),
    );
  ...
  @Get('waitlist/mine')
  listMyWaitlist(@Req() req: { user: { id: string } }) {
    return this.appointments.listMyWaitlist(req.user.id);
  }

  @ApiBearerAuth()
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(UserRole.CUSTOMER)
  @Delete('waitlist/mine/:id')
  leaveMyWaitlist(@Param('id') id: string, @Req() req: { user: { id: string; email: string } }) {
    return this.appointments.leaveWaitlist(id, req.user.email);
  }
  ...
  notifyWaitlistEntry(
    ...
    return this.appointments.notifyWaitlistEntry(id, req.user.orgId, providerScope(req));
  }
  ...
  removeWaitlistEntry(
    ...
    return this.appointments.removeWaitlistEntry(id, req.user.orgId, providerScope(req));
  }
  ...
  clearAllAppointments(
    ...
    return this.appointments.clearAllForOrganization(req.user.orgId, {
      locationId,
